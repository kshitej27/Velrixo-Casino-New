...
# ---------- CONFIG (embedded token as requested) ----------
BOT_TOKEN = "8437765248:AAHsEsW0XknSp8Aa3dc1wylhO3olWj3QPPE"
OWNER_USERNAME = "Velrixo"   # without @
...
def init_db():
    # users: user_id, username, balance, xp, level, last_daily, streak, ref_code, referred_by
    _cur.execute("""
    ...
    # redeem codes: code, amount, uses_allowed, uses_count, creator_id, created_at
    _cur.execute("""
    ...
    # used_codes: code, user_id, used_at
    _cur.execute("""
    ...
    # packets: user_id, amount, last_redeem
    _cur.execute("""
    ...
    _conn.commit()
    # ensure owner is admin if they exist (we'll add admin when owner uses /start)
...
def ensure_user(user_id, username=None):
    ...
    if not _cur.fetchone():
        ...
        _conn.commit()
        # create a small default redeem code tied to user
...
# -- redeem by code (multi-use; uses_allowed==0 => unlimited)
...
# -- admin: makecode (create redeem code). uses_allowed default = 1 (single use) but admin can set >1 or 0 for unlimited
...
# -- admin: addadmin / removeadmin / adminlist
...
# -- givepacket (admin assigns monthly packet to user)
...
# -- user redeem packet (assigned by admin), with 30-day rule
...
# -- admin economy commands: addcash removecash setcash broadcast
...
async def main():
    init_db()
    if not BOT_TOKEN:
        log.error("No BOT_TOKEN found in file.")
        return
